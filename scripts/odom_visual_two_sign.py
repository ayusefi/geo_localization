...
#		r = rospy.Rate(200)
		...
		while not rospy.is_shutdown():
			...
			if (not math.isnan(self.localizer_pos_x)):
				self.odom_visual_pos_x = self.localizer_pos_x
				self.odom_visual_pos_y = self.localizer_pos_y
			else:
				self.odom_visual_pos_x += (self.v_odom * dt * cos(self.yaw) * 100) / math.pow(10, 7)
				self.odom_visual_pos_y += (self.v_odom * dt * sin(self.yaw) * 100) / math.pow(10, 7)
